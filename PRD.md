...
| **FR-UM-05** | **MFA (deferred):** System must support OTP or email-based MFA for sensitive operations. _(Team decision 2026-08-25, during Phase 1: deferred to its own future ticket rather than folded into the identity/RBAC build. Everything else in this module — RBAC, lockout, session revocation, password reset — ships in Phase 1 without it; MFA adds a second, largely independent verification channel (OTP delivery, enrollment/recovery flows) that doesn't block any other FR-UM-\* requirement and deserves its own scoped design pass rather than being rushed alongside the auth foundation.)_ | Must     |
...
| **FR-MOA-02** | **(deferred)** Company name entry must support autocomplete and duplicate detection. _(Team decision during Phase 2: shipped as a plain text input with zod validation only — no duplicate-name warning, no autocomplete. `GET /api/companies?search=` already exists and could back an on-blur duplicate check cheaply; wiring it up was deprioritized behind this phase's Must requirements, which center on FR-CK-04's compliance gate.)_                                                                                                                                                         | Should   |
...
| **FR-MOA-05** | **(deferred)** System must automatically identify MOAs approaching expiration and send email and in-app alerts to the internship coordinator. _(`companyService.getExpiringMoaRecords(daysAhead)` exists as the read primitive a scheduled job would call — the plan explicitly deferred it to Phase 4, per its own comment: "Read primitive for a future Phase 4 cron job." No cron job, email, or in-app alert was wired up this phase.)_                                                                                                                                                          | Should   |
| **FR-MOA-06** | Government-issued business permits must be uploadable and verifiable by faculty or coordinators before the student is activated for internship. _(Implemented as two cooperating pieces rather than one dedicated Company field: the actual upload + Approve/Return review is Checklist Item 7 — `GOVERNMENT_COMPANY_PERMIT` — via Module 3's generic pre-deployment checklist machinery; `Company.isVerified` is a separate, coarser staff-only flag confirming the host company itself, as an organization, is cleared to host interns at all. Both gate different things and both were shipped.)_ | Must     |
...
| **FR-CK-06** | **(deferred)** Checklist item status updates must trigger in-app and email notifications to the student. _(The `notifications` table and its NotificationChannel/NotificationStatus enums exist from Phase 0's schema design but nothing writes to them yet. Every status change IS captured in `audit_logs` — `CHECKLIST_ITEM_UPLOADED`/`CHECKLIST_ITEM_APPROVED`/`CHECKLIST_ITEM_RETURNED` — just not surfaced to the student as a notification.)_ | Should   |
...
| **FR-WP-03** | **(deferred)** System must verify that planned tasks match the student's academic program; if the company has its own work plan format, the system must notify the student and request additional info. _(`plannedTasks` is stored as free-form JSON with no program-match validation or mismatch notification this phase — `submitWorkPlan`'s only enforced rule is FR-WP-06's submit lock.)_ | Should   |
...
| **NFR-SEC-11** | **Row Level Security (defense-in-depth):** All remaining domain tables (everything except `audit_logs`, which NFR-SEC-10 already covers) must have RLS policies matching the RBAC roles in NFR-SEC-01, designed and enabled during Phase 1 alongside the RBAC middleware — not deferred past it. _(Status as of Phase 1: **done**. RLS is enabled with default-deny (no policies) on all 19 remaining tables — `get_advisors` reports zero `rls_disabled_in_public` findings. Team decision 2026-08-25: no per-role policies, only default-deny — the app connects via Prisma as the `postgres`/service role, which bypasses RLS entirely regardless of policy content, so per-role policies would never actually run against real app traffic. Real authorization is 100% app-layer (middleware + `userService.assertCanAccessStudent`). What default-deny RLS actually buys is closing the exposure if the `anon`/publishable key were ever leaked client-side — today that would expose every row in every table; now it exposes none.)_ |
